...
export default function ChatbotFoga() {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed bottom-44 right-4 z-40 w-80 sm:w-96 bg-surface-container-low border border-outline-variant shadow-2xl flex flex-col overflow-hidden"
          style={{ maxHeight: '70vh' }}>
          <div className="bg-primary px-4 py-3 flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-on-primary/20 flex items-center justify-center text-sm font-bold text-on-primary">F</div>
            <div className="flex-1">
              <p className="text-on-primary font-bold text-sm">Assistant Fogatech</p>
              <p className="text-on-primary/60 text-xs">FR · Lingala · Répond en 15min</p>
            </div>
            <a
              href="https://wa.me/242069610635?text=Bonjour%2C%20je%20viens%20du%20chat%20Fogatech%20BTP"
              target="_blank" rel="noreferrer"
              className="text-xs bg-on-primary/20 hover:bg-on-primary/30 text-on-primary px-2 py-1 transition-colors"
            >
              WhatsApp
            </a>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            ...
            {typing && (
              <div className="flex justify-start">
                <div className="bg-surface border border-outline-variant px-4 py-2">
          ...
          <div className="px-3 py-2 flex gap-2 overflow-x-auto border-t border-outline-variant">
            {QUICK_REPLIES.map((q) => (
              <button key={q} onClick={() => sendMessage(q)}
                className="flex-shrink-0 text-xs bg-surface border border-outline-variant text-on-surface-variant hover:border-secondary-container/50 hover:text-secondary-container px-3 py-1.5 rounded-full transition-colors">
                {q}
              </button>
            ))}
          </div>
          ...
          <div className="p-3 border-t border-outline-variant flex gap-2">
            <input
              ...
              className="flex-1 bg-surface border border-outline-variant text-on-surface text-sm px-3 py-2 focus:border-primary outline-none"
            />
            <button
              onClick={() => sendMessage(input)}
              disabled={!input.trim()}
              className="bg-primary disabled:opacity-40 hover:bg-primary-container text-on-primary px-3 py-2 transition-colors"
            >
